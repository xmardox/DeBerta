...
class FlowTransformer:
    ...
    def build_model(self, prefix: str = None):
        ...
        # Hook después del transformer
        m_x = self.classification_head.apply(m_x, prefix)

        # 🔧 Expandimos CLS output (768,) → (1, 768) para que Dense lo acepte
        #m_x = tf.expand_dims(m_x, axis=0)
    ...
    def _load_preprocessed_dataset(self, dataset_name:str,
        ...
        if cache_folder is not None:
            ...
            if os.path.exists(cache_file_path):
                ...
                if "text" not in dataset.columns:
                    print("No se encontró la columna 'text'. Generando a partir de las demás columnas desde cache...")
                    dataset["text"] = dataset.astype(str).agg(" ".join, axis=1)

                # Tokenización si falta
        ...
        if "text" not in dataset.columns:
            print("No se encontró la columna 'text'. Generando a partir de las demás columnas...")
            dataset["text"] = dataset.astype(str).agg(" ".join, axis=1)
        ...
        print("🧪 Revisando columnas antes de crear DataFrame:")
    ...
    def load_dataset(self, dataset_name:str,
        ...
        # Asegurarse de que input_ids y attention_mask estén presentes
    ...
    def evaluate(self, m:keras.Model, batch_size, early_stopping_patience:int=5, epochs:int=100, steps_per_epoch:int=128):
        ...
        def samplewise_to_featurewise(X):
            ...
            for df in X:
                # Cada celda de df["input_ids"] es una lista → convertimos a array
        ...
        for epoch in range(epochs):
            ...
            for step in range(steps_per_epoch):
                ...
                # Aquí calculas y logueas F1 y Balanced Accuracy en cada step.
                ...
                pred_y = m.predict(batch_X, verbose=False).reshape(-1) > 0.5  # predicción binaria
                batch_y = batch_y.reshape(-1)  # Asegúrate de que batch_y sea también un vector 1D

                # Convierte a tipo bool
                pred_y = pred_y.astype(bool)
                batch_y = batch_y.astype(bool)

                # Ahora puedes hacer la operación bitwise
                ...
                # Loguea las métricas en cada paso
                wandb.log({
                    "epoch": epoch,
                    "step": step,
